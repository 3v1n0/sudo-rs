...
fn pam_tty_is_set_when_stdout_is_closed_even_if_stdin_stderr_are_ttys() {
    let (tmp_dir, pam_env_value, _repro_log) = test_temp_paths("stdout-closed-stdin-stderr-ttys");
...
fn pam_tty_with_stdin_here_string_uses_controlling_tty() {
    let (tmp_dir, pam_env_value, _repro_log) = test_temp_paths("stdin-here-string");
...
fn pam_tty_with_background_stdin_here_string_uses_controlling_tty() {
    let (tmp_dir, pam_env_value, repro_log) = test_temp_paths("background-stdin-here-string");
...
expected_tty=$(tty <&3)
set -m
sudo -S -p '' true <<< '{PASSWORD}' >{repro_log} 2>&1 &
sudo_pid=$!
wait "$sudo_pid"
printf '%s=%s\n' '{TEST_ENV_EXPECTED_TTY}' "$expected_tty" >&3
cat {pam_env_value} >&3"#
